...
SectionWidget* ContainerWidget::addSectionContent(const SectionContent::RefPtr& sc, SectionWidget* sw, DropArea area)
{
	...
	auto iconTitleWidget = dynamic_cast<IconTitleWidget*>(data.content->titleWidget());
	if (iconTitleWidget)
	{
		QObject::connect(iconTitleWidget, &IconTitleWidget::CloseButtonClicked, [this, sc]() {emit SectionContentClosed(sc); });
		QObject::connect(iconTitleWidget, &IconTitleWidget::TabClicked, [this, sc]() {raiseSectionContent(sc); });
		QObject::connect(iconTitleWidget, &IconTitleWidget::TabScrolled, [this, sc](int delta)
			{
				auto sw = GetSectionWidgetBySectionContent(sc);
				if (sw)
				{
					int index = sw->currentIndex();
					sw->setCurrentIndex(delta > 0 ? index + 1 : index - 1);
				}
			});
		QObject::connect(iconTitleWidget, &IconTitleWidget::TabShifted, [this, sc](int shift)
			{
				auto sw = GetSectionWidgetBySectionContent(sc);
				if (sw)
				{
					...
					sw->moveContent(index, newIndex);
					sw->setCurrentIndex(newIndex);
				}
			});
		QObject::connect(iconTitleWidget, &IconTitleWidget::CloseAllButtonClicked, [this, sc]()
			{
				auto sw = GetSectionWidgetBySectionContent(sc);
				if (sw)
				{
					for (auto c : sw->contents())
						emit SectionContentClosed(c);
				}
			});
		QObject::connect(iconTitleWidget, &IconTitleWidget::CloseOtherButtonClicked, [this, sc]()
			{
				auto sw = GetSectionWidgetBySectionContent(sc);
				if (sw)
				{
...
SectionWidget* ContainerWidget::GetSectionWidgetBySectionContent(const SectionContent::RefPtr& sc) const
{
	for (auto section : _sections)
	{
		if (section->indexOfContent(sc) != -1)
			return section;
	}
