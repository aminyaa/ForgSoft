...
void SectionWidget::moveContent(int from, int to)
{
	...
	if (to >= _contents.size())
		newTo = _contents.size() - 1;
	else if (to < 0)
		newTo = 0;

	_contents.move(from, newTo);
	_sectionTitles.move(from, newTo);
	_sectionContents.move(from, newTo);

	QLayoutItem* liFrom = NULL;
	liFrom = _tabsLayout->takeAt(from);
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
	_tabsLayout->insertItem(newTo, liFrom);
#else
	_tabsLayout->insertWidget(newTo, liFrom->widget());
	delete liFrom;
	liFrom = NULL;
#endif

	liFrom = _contentsLayout->takeAt(from);
	_contentsLayout->insertWidget(newTo, liFrom->widget());
	delete liFrom;
...
void SectionWidget::setCurrentIndex(int index)
{
	...
	// Set active CONTENT
	_contentsLayout->setCurrentIndex(newIndex);
}
