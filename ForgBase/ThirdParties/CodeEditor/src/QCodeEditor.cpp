...
void QCodeEditor::SetLineNumberEnabled(bool enabled)
{
	if (m_lineNumberArea)
	{
		m_lineNumberArea->SetEnabled(enabled);

		this->update();
	}
}

void QCodeEditor::InsertCompletedCommand(const QString& s, bool isFromCompleter)
{
	...
	auto myKeyEvent = new QKeyEvent(QEvent::KeyPress, Qt::Key_Enter, Qt::NoModifier);
	if (s == "for")
	{
		tc.insertText(" (var i := 0; i < 1; i+=1)");
		setTextCursor(tc);
		keyPressEvent(myKeyEvent);
	...
	else if (s == "while" || s == "if" || s == "else if" || s == "else")
	{
		if (s == "else")
			keyPressEvent(myKeyEvent);
		else
		...
		(
			s == "abs" || s == "ceil" || s == "erf" || s == "erfc" || s == "exp" || s == "expm1" || s == "floor" || s == "frac" || s == "log"
			|| s == "log10" || s == "log1p" || s == "log2" || s == "ncdf" || s == "round" || s == "sgn" || s == "sqrt" || s == "trunc" || s == "acos"
			|| s == "acosh" || s == "asin" || s == "asinh" || s == "atan" || s == "atanh" || s == "cos" || s == "cosh" || s == "cot" || s == "csc" || s == "sec"
			|| s == "sin" || s == "sinc" || s == "sinh" || s == "tan" || s == "tanh" || s == "deg2rad" || s == "deg2grad" || s == "rad2deg" || s == "grad2deg"
			|| s == "avg" || s == "mul" || s == "sum"
			)
	{
		tc.insertText("()");
		...
		(
			s == "atan2" || s == "mod" || s == "logn" || s == "pow" || s == "root" || s == "roundn" || s == "equal" || s == "nequal" || s == "hypot"
			|| s == "shr" || s == "shl" || s == "max" || s == "min" || s == "swap"
			)
	{
		tc.insertText("(, )");
		tc.setPosition(tc.position() - 3);
		setTextCursor(tc);
	}
	else if (s == "clamp" || s == "iclamp" || s == "inrange")
	{
		tc.insertText("(, , )");
...
void QCodeEditor::keyPressEvent(QKeyEvent* e) {
  ...
  const int defaultIndent = tabStopDistance() / fontMetrics().averageCharWidth() / 2;
#else
  const int defaultIndent = tabStopWidth() / fontMetrics().averageCharWidth() / 2;
#endif
