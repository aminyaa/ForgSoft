...
namespace tnbLib
{
	...
	)
	{
		...
		scalar magX = tnbLib::mag(n.x());
		scalar magY = tnbLib::mag(n.y());
		scalar magZ = tnbLib::mag(n.z());
		...
		// HJ, 23/Oct/2007
		if (tnbLib::mag(u1) < intersection::missTol_())
		{
			beta = u0 / u2;
			if
				(
	...
	)
	{
		...
		// Calculate distance.
		// Note: tnbLib::mag used since truncation error causes negative distances
		...
		(
			...
			tnbLib::sqrt
			(
				tnbLib::mag(a*s*s + 2 * b*s*t + c * t*t + 2 * d*s + 2 * e*t + f)
			),
			!inside
		);
	}
	...
	inline scalar triangle<Point, PointRef>::circumRadius() const
	{
		...
		else
		{
			...
			return 0.5*tnbLib::sqrt(min(GREAT, max(scalar(0), a)));
		}
	}
	...
	) const
	{
		...
		if (tnbLib::mag(alpha + beta - 1) <= tol)
		{
			// On edge between vert 1-2 (=edge 1)

			if (tnbLib::mag(alpha) <= tol)
			{
				nearType = POINT;
				nearLabel = 2;
			}
			else if (tnbLib::mag(beta) <= tol)
			{
				nearType = POINT;
				nearLabel = 1;
			}
		...
		else if (tnbLib::mag(alpha) <= tol)
		{
			// On edge between vert 2-0 (=edge 2)

			if (tnbLib::mag(beta) <= tol)
			{
				nearType = POINT;
				nearLabel = 0;
			}
			else if (tnbLib::mag(beta - 1) <= tol)
			{
				nearType = POINT;
				nearLabel = 2;
			}
		...
		else if (tnbLib::mag(beta) <= tol)
		{
			...
			else if (tnbLib::mag(alpha - 1) <= tol)
			{
				nearType = POINT;
				nearLabel = 1;
			}
