...
namespace tnbLib
{
	...
	line<Point, PointRef>::line(const Point& start, const Point& end)
		:
		a_(start),
		b_(end)
	{}
	...
	template<class Point, class PointRef>
	line<Point, PointRef>::line(Istream& is)
	{
	...
	template<class Point, class PointRef>
	PointRef line<Point, PointRef>::start() const
	{
		return a_;
	}

	template<class Point, class PointRef>
	PointRef line<Point, PointRef>::end() const
	{
		return b_;
	}
	...
	template<class Point, class PointRef>
	Point line<Point, PointRef>::centre() const
	{
		return 0.5*(a_ + b_);
	}
	...
	template<class Point, class PointRef>
	scalar line<Point, PointRef>::mag() const
	{
		return ::tnbLib::mag(vec());
	}


	template<class Point, class PointRef>
	Point line<Point, PointRef>::vec() const
	{
		return b_ - a_;
	}
	...
	PointHit<Point> line<Point, PointRef>::nearestDist(const Point& p) const
	{
		...
		if (c1 <= 0)
		{
			return PointHit<Point>(false, a_, tnbLib::mag(p - a_), true);
		}
		...
		if (c2 <= c1)
		{
			return PointHit<Point>(false, b_, tnbLib::mag(p - b_), true);
		}
		...
		return PointHit<Point>(true, pb, tnbLib::mag(p - pb), false);
	}
	...
	) const
	{
		...
		return tnbLib::mag(thisPt - edgePt);
	}
