...
namespace tnbLib
{
	...
	template<class Cmpt>
	Tensor2D<Cmpt>::Tensor2D()
	{}


	template<class Cmpt>
	Tensor2D<Cmpt>::Tensor2D(const VectorSpace<Tensor2D<Cmpt>, Cmpt, 4>& vs)
		:
	...
	template<class Cmpt>
	Tensor2D<Cmpt>::Tensor2D(const SymmTensor2D<Cmpt>& st)
	{
	...
	template<class Cmpt>
	Tensor2D<Cmpt>::Tensor2D(const SphericalTensor2D<Cmpt>& st)
	{
	...
	template<class Cmpt>
	Tensor2D<Cmpt>::Tensor2D(Istream& is)
		:
	...
	template<class Cmpt>
	Vector2D<Cmpt> Tensor2D<Cmpt>::x() const
	{
		return Vector2D<Cmpt>(this->v_[XX], this->v_[XY]);
	}

	template<class Cmpt>
	Vector2D<Cmpt> Tensor2D<Cmpt>::y() const
	{
	...
	template<class Cmpt>
	const Cmpt& Tensor2D<Cmpt>::xx() const
	{
		return this->v_[XX];
	}
	...
	template<class Cmpt>
	const Cmpt& Tensor2D<Cmpt>::xy() const
	{
		return this->v_[XY];
	}
	...
	template<class Cmpt>
	const Cmpt& Tensor2D<Cmpt>::yx() const
	{
		return this->v_[YX];
	}
	...
	template<class Cmpt>
	const Cmpt& Tensor2D<Cmpt>::yy() const
	{
		return this->v_[YY];
	}
	...
	template<class Cmpt>
	const Cmpt& Tensor2D<Cmpt>::operator()
		(
			const direction i,
			const direction j
			) const
	...
	template<class Cmpt>
	Tensor2D<Cmpt> Tensor2D<Cmpt>::T() const
	{
	...
	template<class Cmpt>
	void Tensor2D<Cmpt>::operator=(const SymmTensor2D<Cmpt>& st)
	{
		this->v_[XX] = st.xx(); this->v_[XY] = st.xy();
		this->v_[YX] = st.xy(); this->v_[YY] = st.yy();
	}
	...
	template<class Cmpt>
	void Tensor2D<Cmpt>::operator=(const SphericalTensor2D<Cmpt>& st)
	{
		this->v_[XX] = st.ii(); this->v_[XY] = 0;
		this->v_[YX] = 0; this->v_[YY] = st.ii();
	}
