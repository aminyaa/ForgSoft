...
namespace tnbLib
{
	...
	namespace debug
	{
		...
		template<class T>
		tnbLib::debug::controlSwitches<T>::controlSwitches
		(
		...
		(
			...
			<
			std::string,
			std::list<tnbLib::debug::controlSwitches<T> *>
			>* mapListSwitchesPtr,
		...
		)
		{
			...
			if (mapListSwitchesPtr)
			{
				...
				for (it = mapListSwitches.begin(); it != mapListSwitches.end(); it++)
				{
					...
					std::list<tnbLib::debug::controlSwitches<T> *> swList = it->second;
					tnbLib::debug::controlSwitches<T>& value = *(swList.front());
					...
					typename
						std::list<tnbLib::debug::controlSwitches<T> *>::iterator itL;
