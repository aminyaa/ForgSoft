	...
	: FrgBaseSceneTreeItem(title, parent, parentTree, parentMainwindow)
{
	...
	connect(theOKButton_, SIGNAL(clicked(bool)), this, SLOT(OKButtonClickedSlot(bool)));

	//GetProperties()->Add
}

void ForgBaseLib::NihadVesselSceneTreeItem::OKButtonClickedSlot(bool checked)
{

	thePartsPointer_.at(0)->theGeometryPointer_->thePatch_->Discrete();
...
void ForgBaseLib::NihadVesselSceneTreeItem::CreateActor()
{
	...
	for
		(
			TopExp_Explorer Explorer(thePartsPointer_.at(0)->theGeometryPointer_->thePatch_->Entity(), TopAbs_FACE);
			Explorer.More();
			Explorer.Next(), it++
			)
	{
