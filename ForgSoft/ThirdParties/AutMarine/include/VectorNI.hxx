...
template<class Cmpt, int length>
inline AutLib::VectorN<Cmpt, length>::VectorN()
{}

template<class Cmpt, int length>
inline AutLib::VectorN<Cmpt, length>::VectorN(const VectorSpace<VectorN<Cmpt, length>, Cmpt, length>& vs)
	: VectorSpace<VectorN<Cmpt, length>, Cmpt, length>(vs)
{}

template<class Cmpt, int length>
inline AutLib::VectorN<Cmpt, length>::VectorN(const Cmpt & vx)
{
	VectorSpaceOps<VectorN<Cmpt, length>::nComponents, 0>::eqOpS(*this, vx, eqOp<Cmpt>());
}

template<class Cmpt, int length>
inline AutLib::VectorN<Cmpt, length>::VectorN(Istream & is)
...
template<class Cmpt, int length>
inline AutLib::VectorN<Cmpt, length> AutLib::VectorN<Cmpt, length>::cmptMultiply(const VectorN<Cmpt, length>& v)
{
...
namespace AutLib
{
	...
	template<class Cmpt, int length>
	inline VectorN<Cmpt, length> transform(const tensor& tt, const VectorN<Cmpt, length>& v)
	{
		return v;
	}
	...
	template <class Cmpt, int length>
	inline Cmpt operator&(const VectorN<Cmpt, length>& v1, const VectorN<Cmpt, length>& v2)
	{
	...
	template<class Cmpt, int length>
	inline VectorN<Cmpt, length> operator/(const scalar s, const VectorN<Cmpt, length>& v)
	{
		VectorN<Cmpt, length> res;
	...
	template<class Cmpt, int length>
	inline VectorN<Cmpt, length> scaleRow(const VectorN<Cmpt, length>& v, const Cmpt& c)
	{
		return v * c;
	}
